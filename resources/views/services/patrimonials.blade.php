	...
	<div class="panel-body" style="display:none;">
		<div class="table-responsive">
        	<table class="table table-bordered table-striped" id="table_patrimonials" data-toggle="table" data-toolbar="#filter-bar" data-show-toggle="false" data-search="false" data-show-filter="false" data-show-columns="false" data-show-export="false" data-pagination="false" data-click-to-select="true" data-page-list="[10, 20, 50, 100, All]" data-toolbar="#filter-bar"> 
				...
				<tbody>
				    @foreach($service_patrimonials as $service_patrimonial)
				        <tr>
				            <td><a href="{!! route('patrimonials.show', [$service_patrimonial->patrimonial_id]) !!}">{{ $service_patrimonial->patrimonial->code }}</a></td>
				            <td>{{ $service_patrimonial->patrimonial->code_old }}</td>
				            <td>{{ $service_patrimonial->patrimonial->patrimonial_type->code }}</td>
				            <td>{{ $service_patrimonial->patrimonial->patrimonial_sub_type->code }}</td>
				            <td>{{ $service_patrimonial->patrimonial->description }}</td>
				            <td>{{ $service_patrimonial->patrimonial->management_unit->region->description }}</td>
				             <td><a href="{!! route('management_units.show', [$service_patrimonial->patrimonial->management_unit_id]) !!}" title="{{ $service_patrimonial->patrimonial->management_unit->description }}">{{ $service_patrimonial->patrimonial->management_unit->code }}</a></td>
				            <td>{{ $service_patrimonial->patrimonial->patrimonial_sector->description }}</td>
				            <td>{{ $service_patrimonial->patrimonial->patrimonial_sub_sector->description }}</td>
				            <td>{{ $service_patrimonial->patrimonial->patrimonial_status->description }}</td>
				            <td>{{ $service_patrimonial->patrimonial->patrimonial_status_date->format('d/m/Y') }}</td>
				        </tr>
